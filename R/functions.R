...
GenerateVariantLevelFS <- function(bam_file, variants, ref_set) {
  ## generates a variant (VFS) and wildtype (WFS) score for each variant
  ## SNVs and indels are supported

  library(dplyr)

  bam_ranges <- GetBamReadsAtVariantSites(bam_file, variants)
...
GenerateTumourInformedFS <- function(bam_file, variants, ref_set) {
...
CalculateScoreFromLengths <- function(lengths, ref_set) {
  ## "lengths" must be passed as a frequency table

  if (sum(lengths) == 0) {
    return(NA)
  }
  score <- weighted.mean(x = ref_set[as.numeric(names(lengths))], w = lengths)
  return(score)
}
